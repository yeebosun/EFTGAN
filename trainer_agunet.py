...
data_dir = os.path.join(current_file_dir, 'HEA_Data/change_fcc')
...
def load_hea_data_single_file(split_type=2, is_validate=True, batch_size=64, RANDOM_SEED=1454880):
    """
    ...
    current_file_dir = os.path.dirname(__file__)
    if split_type == 0:
        data_file = os.path.normpath(os.path.join(current_file_dir, 'HEA_Data/Out_labels/Database_fcc.xlsx'))
        print('loading (2+3+4+5)fcc component HEA data with randomly splitting all the data into training/testing set\t'
    ...
    total_dataset = AGUDataset(txt_dir=data_dir, label_name=data_file)
...
def load_hea_data(split_type=0, is_validate=True):
    ...
    if split_type == 0:
        ...
        raise AssertionError('Currently, we only supprot split=0/1 for randomly splitting and splitting based on '
                             'the number of  components')
    train_dataset = AGUDataset(txt_dir=data_dir, label_name=train_file)
    test_dataset = AGUDataset(txt_dir=data_dir, label_name=test_file)
    validate_loader = None  # declare the validate loader
    if is_validate:
...
def validate_model_hea(model, loader, tasks, transforms):
    ...
    with torch.no_grad():
        for data, labels in loader:
            data=data.to(device)
            labels_on_gpu = {key: torch.tensor(value, dtype=torch.float32).to(device) for key, value in labels.items()}
            out = model(data)
            ...
            for ix, task in enumerate(tasks):
                y = labels_on_gpu[task]  # obtain the true labels of the data set.
                yhat = out[ix]
...
def train(model, criterion):
    ...
    for epoch in range(num_epochs):
        for index, (data, labels) in enumerate(train_loader):
            ...
            labels_on_gpu = {key: torch.tensor(value, dtype=torch.float32).to(device, non_blocking=True) for key, value in labels.items()}
            # print(data.device)
            out = model(data)
            # out = [y.squeeze() for y in out]
            y_list = []
            ...
            for ix, task in enumerate(args.task):
                ...
                if len(args.transform):  # transform is not empety.
                    assert len(args.transform) == len(args.task), 'the length of task and transform parameters must be ' \
                                                                  'equal. '
                    y = data_transform.transform(args.transform[ix], labels_on_gpu[task], inverse_trans=False)
                y_list.append(y)
    ...
    if args.save_model:
        ...
        model_name = 'agu_mtl_' + str(len(args.task))
...
def test():
    ...
    model_name = './saved_agumodels/agu_mtl_1_eform_HEA_800_b0_best.pt'
...
def save_to_excel(out_true, out_pred, tasks):
    """
    Save the true and predicted values to an Excel file for each task.

    Args:
        out_true (list): List of arrays containing the true values for each task.
        out_pred (list): List of arrays containing the predicted values for each task.
        tasks (list): List of task names.
    """
    for i, task in enumerate(tasks):
        # Create a DataFrame for the current task
        df = pd.DataFrame({
            'True Values': out_true[i],
            'Predicted Values': out_pred[i]
        })

        # Define the Excel file name
        file_name = f'output_{task}.xlsx'

        # Save the DataFrame to an Excel file
        df.to_excel(file_name, index=False)
        print(f'Saved {task} data to {file_name}')


# Example usage
# Assuming out_true, out_pred, and args.task are defined and populated
# save_to_excel(out_true, out_pred, args.task)
...
def plot():
    ...
    #save_to_excel(out_true, out_pred, args.task)
    save_to_excel(out_true_train, out_pred_train, args.task)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, default='agunet', help='the name of the ML model')
