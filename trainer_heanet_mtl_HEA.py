...
def load_hea_data(split_type=0, is_validate=True):
    ...
    if split_type == 0:
        ...
        train_file = './HEA_Data/Out_labels/Database.xlsx'
    ...
    print('The data size is {}'.format(len(train_dataset)))
    ...
    if is_validate:
        ...
        validate_loader = DataLoader(dataset=train_dataset, batch_size=args.batch_size,
                                     sampler=SequentialSampler(v_seq))
        print('The number of the train size is {}, validation size is {}'.format(len(t_seq), len(v_seq)))
    else:
...
def test():
    ...
    model_name = './saved_models_mtl_HEA/mtl_1_rmsd_HEA_500_a0_best.pt'
    ...
    for i in range(len(args.task)):
        mae_t = mean_absolute_error(out_true[i], out_pred[i])
        r2_t = r2_score(out_true[i], out_pred[i])
        mae_train = mean_absolute_error(out_true_train[i], out_pred_train[i])
        r2_train = r2_score(out_true_train[i], out_pred_train[i])
        print('The MAE of {} is {}; The R2 score is {}'.format(args.task[i], mae_t, r2_t))
        print('The MAE of {} in the training set is {}; The R2 score is {}'.format(args.task[i], mae_train, r2_train))
        df = pd.DataFrame({
            'True Values': out_true[i],
            'Predicted Values': out_pred[i]
        })
        file_name = f'task_{i}_test_.xlsx'
        df.to_excel(file_name, index=False)
        df = pd.DataFrame({
            'True Values': out_true_train[i],
            'Predicted Values': out_pred_train[i]
        })
        file_name = f'task_{i}_train.xlsx'
        df.to_excel(file_name, index=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--processed_data', action='store_false',
